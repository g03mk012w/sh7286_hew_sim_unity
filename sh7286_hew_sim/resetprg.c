...
//	_s1ptr=NULL;					// Remove the comment when you use strtok()
	...
	//スタック領域初期化：スタック消費量がわかるように0xeeで埋める
	{
		const unsigned int* sectop_S = __sectop("S");
		const unsigned int* secend_S = __secend("S");
		const unsigned int* secfil_S = secend_S - 0x20;  //ちょっと使っている可能性があるため0x40分は埋めない
		unsigned int* sec_p;

		for (sec_p = sectop_S; sec_p < secfil_S; sec_p++) {
			*sec_p = 0xeeeeeeeeu;
		}
	}

	//ヒープ領域初期化：消費量がわかるように0xaaで埋める
	{
		const unsigned int* sectop_H = __sectop("BHeap");
		const unsigned int* secend_H = __secend("BHeap");
		unsigned int* sec_p;

		for (sec_p = sectop_H; sec_p < secend_H; sec_p++) {
			*sec_p = 0xaaaaaaaau;
		}
	}
